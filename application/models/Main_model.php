...
Class Main_model extends CI_Model {
	...
	public function recent_report_by_id($id){
		...
		$query = $this->db->query('SELECT * FROM `quantitave_input` WHERE id = "'. $id .'" ORDER BY  id DESC LIMIT 3');
		if($query->num_rows() > 0)
		{
			foreach($query->result() as $key=>$item)
			{
	...
	public function recent_reports($name){
		...
		// }
		// return false;
		$query = $this->db->query('SELECT * FROM `quantitave_input` WHERE company_name = "'. $name .'" ORDER BY  id DESC LIMIT 3');
		if($query->num_rows() > 0)
		{
			foreach($query->result() as $key=>$item)
			{
	...
	public function get_repots_list(){
		...
		$query = $this->db->query('SELECT `company_information`.`entity_name`,`quantitave_input`.* FROM `quantitave_input`,`company_information` WHERE STATUS = "0" AND company_name = `company_information`.`id` ORDER BY id DESC');
		if($query->num_rows() > 0)
		{
			foreach($query->result() as $key=>$item)
			{
	...
	public function company_country($id){

		$query = $this->db->query('SELECT country FROM `company_information` WHERE id = "'.$id.'"');
		if($query->num_rows() > 0)
		{
			foreach($query->result() as $key=>$item)
			{
